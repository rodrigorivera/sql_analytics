...
- include cost based KPIs (e.g. Inventory valued at cost price)
- adjust sold items by cancellation and return forecast (see Return Rate forecast logic script)
ATTENTION: check inventory type filter! Possibly needs to be adjusted per venture since inventory types are named differently!
*/

--*****************************************************************************************************************


define v_observe_period = 10; -- this is the number of days that we calibrate the sales rate on
define v_missing_act_at_factor = 14; -- if activated at is null this value will be added to created at to approximate activated at. SHould be the average time between created at and activated at
define v_observation_point = 1; -- this is the stock date we are observing
define v_venture = 'namshi_ae' -- this is the schema name of the venture
...
select 
	...
	visibility_period, 
	case 
		when sold_items_before_return/visibility_period = 0 then 999 -- when sales rate is 0 then it takes an infinite time to deplete the stock
...
case when days_without_stock is null then &v_observe_period -- when days without stock is null, then the SKU is on stock in the full observation period
...
select sku, 
case when round(sysdate - activated_at,0) > &v_observe_period then &v_observe_period -- days active is larger than observation period
when activated_at is null and round(sysdate - created_at + &v_missing_act_at_factor,0) <= 21 then round(sysdate - created_at + &v_missing_act_at_factor,0) -- when activated_at is null we will approximate activated at by taking created at and adding the average days for a product to switch from created at to activated at
when activated_at is null and round(sysdate - created_at + &v_missing_act_at_factor,0) > 21 then &v_observe_period
...
and (inventory_type = 'Own Warehouse' or inventory_type is null) -- adjusted for Namshi!! Only own warehouse goods are being analysed
and type_id = 'simple'
) days_active_sku
left join
(
/* Determine the lenght of the period that a simple SKU has been out of stock and thus was not visible*/
select sku,count(stock_date) as days_without_stock from &v_venture.stock
where round(sysdate - stock_date,0) <= &v_observe_period
...
from &v_venture.bob_sales_order_item
--where dg_end_date > sysdate -- needs to be eliminated for miniDWH
and round(sysdate - created_at,0) <= &v_observe_period -- only sales within observation period
...
from &v_venture.stock
--where dg_end_date > sysdate -- needs to be eliminated for miniDWH
and round(sysdate - stock_date,0) = &v_observation_point -- needs to be transformed into a variable &v_observation_point. For some reason it did not work while it worked with &observation_period
